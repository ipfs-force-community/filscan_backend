...
import (
	...
	redisutils "gitlab.forceup.in/fil-data-factory/filscan-backend/pkg/redis/utils"
)
...
func NewRedisLock(key string, client LockClient, opts ...LockOption) *RedisLock {
	r := RedisLock{
		key:    key,
		token:  redisutils.GetProcessAndGoroutineIDStr(),
		client: client,
	}
...
func (r *RedisLock) tryLock(ctx context.Context) error {
	// 首先查询锁是否属于自己
	reply, err := r.client.SetNEX(r.getLockKey(), r.token, r.expireSeconds)
	if err != nil {
		return err
	}
	if reply != 1 {
...
func (r *RedisLock) DelayExpire(ctx context.Context, expireSeconds int64) error {
	...
	reply, err := r.client.Eval(LuaCheckAndExpireDistributionLock, 1, keysAndArgs)
	if err != nil {
		return err
	}
...
func (r *RedisLock) Unlock(ctx context.Context) error {
	...
	reply, err := r.client.Eval(LuaCheckAndDeleteDistributionLock, 1, keysAndArgs)
	if err != nil {
		return err
	}
