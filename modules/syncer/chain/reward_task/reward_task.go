...
func (m MinerRewardTask) Exec(ctx *syncer.Context) (err error) {
	...
	for _, r := range rewards {
		debuglog.Logger.Infof("reward task, miner: %s, epoch: %d,block count: %d, reward: %s", r.Id.Miner, r.Id.Epoch, r.BlockCount, r.TotalBlockReward)
	}

	for _, wc := range winCounts {
		debuglog.Logger.Infof("reward task, miner: %s,  win count: %d", wc.Id, wc.TotalWinCount)
	}
